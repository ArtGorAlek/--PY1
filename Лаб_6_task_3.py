...
def to_csv_file(filename, headers: list, rows: list, delimiter=",", new_line="\n") -> None:
    with open(filename, 'w') as output_file:
        for i in range(len(headers)):
            output_file.write(headers[i] + delimiter if i != len(headers)-1 else headers[i])  # записываем в файл элементы headers и если это последний элемент то разделитель не ставится
